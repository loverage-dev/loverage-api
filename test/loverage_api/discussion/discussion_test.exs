defmodule Loverage.DiscussionTest do
  ...
  describe "categories" do
    ...
    test "list_categories/0 returns all categories" do
      ...
      assert Discussion.list_categories() == [category]
    end

    test "get_category!/1 returns the category with given id" do
      category = category_fixture()
      assert Discussion.get_category!(category.id) == category
    end

    test "create_category/1 with valid data creates a category" do
      assert {:ok, %Category{} = category} = Discussion.create_category(@valid_attrs)
      assert category.name == "some name"
    end

    test "create_category/1 with invalid data returns error changeset" do
      assert {:error, %Ecto.Changeset{}} = Discussion.create_category(@invalid_attrs)
    end

    test "update_category/2 with valid data updates the category" do
      category = category_fixture()
      assert {:ok, category} = Discussion.update_category(category, @update_attrs)
    ...
    test "update_category/2 with invalid data returns error changeset" do
      category = category_fixture()
      assert {:error, %Ecto.Changeset{}} = Discussion.update_category(category, @invalid_attrs)
      assert category == Discussion.get_category!(category.id)
    end

    test "delete_category/1 deletes the category" do
      category = category_fixture()
      assert {:ok, %Category{}} = Discussion.delete_category(category)
      assert_raise Ecto.NoResultsError, fn -> Discussion.get_category!(category.id) end
    end

    test "change_category/1 returns a category changeset" do
      category = category_fixture()
      assert %Ecto.Changeset{} = Discussion.change_category(category)
    end
  end
end
